...
## Region
if [ -z "${ALIASES}" ]; then
  echo >&2 'ERROR: ALIASES variable is empty. aborting...'
  exit 1
fi
...
root="$ROOT_PATH" command="$COMMAND" components="$COMPONENTS"  aliases="$ALIASES" python ${ROOT_PATH}/modules/landing_zone/scripts/config.py
